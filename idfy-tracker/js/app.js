/* ==========================================================================
   ...
   Main application controller: state, navigation, rendering, CRUD, modals.
   ========================================================================== */
...
var App = (function () {
  ...
  function init() {
    ...
    if (loaded && loaded.length) {
      ...
      state.projects = Data.sampleProjects();
      persist();
    }
    ...
    attachGlobalSearch();
    attachModalBackdrop();
    $("#btnAddProject").addEventListener("click", function () { openProjectForm(null); });
    ...
    $("#importFileInput").addEventListener("change", function (e) {
      ...
      Storage.importJSON(file, function (err, data) {
        ...
        persist();
        renderPage();
        alert("Import complete — " + data.length + " projects loaded.");
      });
      e.target.value = "";
    });
  ...
  function matchesFilters(p) {
    ...
    if (state.search) {
      var haystack = [p.client, p.projectName, (p.modules || []).join(" "), p.owner, p.environment, p.cloudProvider]
        .join(" ").toLowerCase();
  ...
  function filteredProjects() {
    return state.projects.filter(matchesFilters);
  }
  ...
  function renderDashboard(main) {
    ...
    projects.forEach(function (p) {
      ...
      clientWaiting += a.clientWaiting;
      internalWaiting += a.internalWaiting;
      activeWork += a.activeWork;
    ...
    html += '<div class="section-title">At a Glance — Kanban</div>';
    html += '<div id="dashboardKanban"></div>';

    main.innerHTML = html;
    Kanban.render($("#dashboardKanban"), projects.slice(0, 30));
  }
  ...
  function filterBarHtml() {
    ...
    function opts(list, current) {
      return ["All"].concat(list).map(function (v) {
  ...
  function renderKanbanPage(main) {
    ...
    Kanban.render($("#kanbanContainer"), filteredProjects());
  }
  ...
  function renderProjectsPage(main) {
    ...
    if (!projects.length) {
      ...
      projects.forEach(function (p) {
        ...
        html += '<td><button class="link-btn link-danger" data-delete-project="' + p.id + '">Delete</button></td>';
        html += "</tr>";
      });
    }
    ...
    $all("[data-open-project]", main).forEach(function (el) {
      el.addEventListener("click", function (e) { e.preventDefault(); openProjectDrawer(el.getAttribute("data-open-project")); });
    });
    $all("[data-delete-project]", main).forEach(function (el) {
      el.addEventListener("click", function () { confirmDeleteProject(el.getAttribute("data-delete-project")); });
    });
  }
  ...
  function renderTimelinePage(main) {
    ...
    html += '<input type="date" id="tlDateFrom" title="From date">';
    html += '<input type="date" id="tlDateTo" title="To date">';
    html += '<button class="btn btn-ghost btn-sm" id="btnClearTlFilters">Clear filters</button>';
    html += "</div>";
    ...
    function apply() {
      state.globalTimelineFilters = {
        client: $("#tlClient").value,
        ownerType: $("#tlOwnerType").value,
        activityType: $("#tlActivityType").value,
        status: $("#tlStatus").value,
        dateFrom: $("#tlDateFrom").value,
        dateTo: $("#tlDateTo").value
      };
      Timeline.renderGlobalTimeline($("#globalTimelineContainer"), state.projects, state.globalTimelineFilters);
    }

    ["tlClient", "tlOwnerType", "tlActivityType", "tlStatus", "tlDateFrom", "tlDateTo"].forEach(function (id) {
  ...
  function renderSettingsPage(main) {
    var html = '<div class="page-header"><h1>Settings</h1><p class="page-subtitle">Data management for this browser</p></div>';
    html += '<div class="settings-card">';
    html += "<h3>Data storage</h3>";
    html += "<p>All data is stored locally in this browser via <code>localStorage</code>. Nothing is sent to a server.</p>";
    html += '<div class="settings-actions">';
    html += '<button class="btn btn-secondary" id="btnExport2">Export data (JSON)</button>';
    html += '<button class="btn btn-secondary" id="btnImport2">Import data (JSON)</button>';
    html += '<button class="btn btn-secondary" id="btnResetSample2">Reset to sample data</button>';
    html += '<button class="btn btn-danger" id="btnClearAll2">Clear all data</button>';
    html += "</div></div>";
    ...
    $("#btnExport2").addEventListener("click", function () { Storage.exportJSON(state.projects); });
    $("#btnImport2").addEventListener("click", function () { $("#importFileInput").click(); });
  ...
  function renderDrawer() {
    ...
    html += '<div class="drawer-section"><div class="drawer-section-title">Project Information</div>';
    ...
    html += '<div class="drawer-section"><div class="drawer-section-title">Modules</div>';
    html += '<div class="chip-row">' + Kanban.moduleChips(project.modules, 10) + "</div></div>";
    ...
    html += '<div class="drawer-section"><div class="drawer-section-title">Waiting-Time Analytics</div>';
    ...
    html += '<div class="drawer-section-title-row"><div class="drawer-section-title">Project Timeline</div><button class="btn btn-primary btn-sm" id="btnAddActivity">+ Add Activity</button></div>';
    html += Timeline.renderProjectTimeline(project);
    html += "</div>";

    html += '<div class="drawer-section"><div class="drawer-section-title">Activity Log</div>';
    html += '<div class="audit-log">';
    if (!project.auditLog || !project.auditLog.length) {
      html += '<div class="empty-state">No log entries yet.</div>';
    } else {
      project.auditLog.forEach(function (l) {
        html += '<div class="audit-item"><span class="audit-date">' + Data.formatDate(l.date) + '</span><span class="audit-text">' + esc(l.text) + "</span></div>";
      });
    }
    html += "</div></div>";
  ...
  function miniStat(label, value) {
    return '<div class="mini-stat"><div class="mini-stat-value">' + value + '</div><div class="mini-stat-label">' + esc(label) + "</div></div>";
  }

  function attachDrawerEvents(project) {
    ...
    $("#btnAddActivity").addEventListener("click", function () { openActivityForm(project.id, null); });
    $all("[data-edit-activity]").forEach(function (btn) {
      btn.addEventListener("click", function () { openActivityForm(project.id, btn.getAttribute("data-edit-activity")); });
    });
    $all("[data-delete-activity]").forEach(function (btn) {
      btn.addEventListener("click", function () { confirmDeleteActivity(project.id, btn.getAttribute("data-delete-activity")); });
    });
  }
  ...
  function openProjectForm(projectId) {
    ...
    html += formField("Status", selectHtml("f_status", Data.STATUSES.map(function (s) { return s.label; }),
      (Data.STATUSES.filter(function (s) { return s.key === project.status; })[0] || {}).label || "Backlog", true));
    ...
    function toggleCloudRow() {
      var row = $("#f_cloudProviderRow");
      if (row) row.style.display = $("#f_environment").value === "Cloud" ? "" : "none";
    }
    ...
    $("#projectForm").addEventListener("submit", function (e) {
      ...
      var payload = {
        ...
        projectName: $("#f_projectName").value.trim(),
        projectType: $("#f_projectType").value,
        environment: $("#f_environment").value,
        cloudProvider: $("#f_environment").value === "Cloud" ? $("#f_cloudProvider").value : "",
        infrastructureOwnership: $("#f_infra").value,
        owner: $("#f_owner").value.trim(),
        startDate: $("#f_startDate").value,
        targetDate: $("#f_targetDate").value,
        status: statusKey,
      ...
      persist();
      closeModal();
      if (state.page === "dashboard" || state.page === "kanban" || state.page === "projects") renderPage();
  ...
  function formField(label, inputHtml, rowId, fullWidth) {
    return '<div class="form-field' + (fullWidth ? " form-field-full" : "") + '"' + (rowId ? ' id="' + rowId + '"' : "") + ">" +
      '<label>' + esc(label) + "</label>" + inputHtml + "</div>";
  }
  ...
  function openActivityForm(projectId, activityId) {
    ...
    $("#activityForm").addEventListener("submit", function (e) {
      ...
      var payload = {
        date: $("#a_date").value,
        activityType: $("#a_type").value,
        description: $("#a_description").value.trim(),
        ownerType: $("#a_ownerType").value,
        owner: $("#a_owner").value.trim(),
        dependencySide: $("#a_side").value,
        requestedBy: $("#a_requestedBy").value.trim(),
        requestedDate: $("#a_requestedDate").value,
        expectedDate: $("#a_expectedDate").value,
        receivedDate: $("#a_receivedDate").value,
        status: $("#a_status").value,
        impact: $("#a_impact").value.trim(),
        relatedPhase: $("#a_phase").value.trim(),
      ...
      if (isEdit) {
        ...
        addAudit(project, "Activity updated: " + payload.description);
      } else {
        payload.id = Data.generateId("act");
        project.activities.push(payload);
        addAudit(project, "Activity added: " + payload.description);
      }

      persist();
      closeModal();
      renderDrawer();
      if (state.page === "timeline" || state.page === "analytics" || state.page === "dashboard") renderPage();
    });
  }
